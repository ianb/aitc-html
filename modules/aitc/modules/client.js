...
AitcClient.prototype = {
  ...
  remoteUninstall: function remoteUninstall(app, cb) {
    ...
    var record = this._makeRemoteApp(app);
    record.deleted = true;
  ...
  getApps: function getApps(cb) {
    ...
    var req = new TokenAuthenticatedRESTRequest(uri, this.token);
  ...
  _putApp: function _putApp(app, cb) {
    ...
    var uri = this._makeAppURI(app.origin);
    var req = new TokenAuthenticatedRESTRequest(uri, this.token);
    if (app.modified) {
    ...
    req.put(JSON.stringify(app), function _tryPuttingAppFinished(error) {
      ...
      switch (req.response.status) {
        ...
        case 412:
        case 413:
          var msg = "_putApp returned: " + req.response.status;
          self._log.warn(msg);
          err = new Error(msg);
  ...
  _makeAppURI: function _makeAppURI(origin) {
    var part = CommonUtils.encodeBase64URL(
