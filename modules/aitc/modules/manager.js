...
AitcManager.prototype = {
  ...
  appEvent: function appEvent(type, app) {
    ...
    var self = this;
    var obj = {type: type, app: app, retries: 0, lastTime: 0};
  ...
  _startPoll: function _startPoll() {
    ...
    var self = this;
    var getFreq = PREFS.get("manager.getFreq");
  ...
  _processQueue: function _processPutQueue() {
    ...
    function _reschedule() {
      ...
      var obj = self._pending.peek();
      var cTime = new Date().getTime();
      var freq = PREFS.get("manager.putFreq");
  ...
  _getToken: function _getToken(assertion, cb) {
    var url = this.TOKEN_SERVER + "/1.0/aitc/1.0";
    var client = new TokenServerClient();
    ...
    client.getTokenFromBrowserIDAssertion(url, assertion, function(err, tok) {
      ...
      var msg = "Unknown error in _getToken " + err.message;
      self._log.error(msg);
  ...
  _makeClient: function makeClient(cb, login, win) {
    ...
    if (this._client) {
      var msg = "Client already exists, not creating";
      self._log.info(msg);
      cb(new Error(msg), null);
      return;
    }
